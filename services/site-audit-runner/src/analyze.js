...
import { evaluateHsts } from "./hsts-policy.js";
...
export function analyzeSnapshot(snapshot, pagespeed = null, technicalProfile = {}, metadata = {}) {
  ...
  checks.push(makeCheck({
    ...
    observation: `Znaleziono ${formatPolishCount(inspection.h1Count, { one: "nagłówek H1", few: "nagłówki H1", many: "nagłówków H1" })}.`,
  ...
  checks.push(makeCheck({
    ...
    observation: crawledPages.length ? `Sprawdzono ${formatPolishCount(crawledPages.length, { one: "podstronę", few: "podstrony", many: "podstron" })}; ${formatPolishCount(brokenPages.length, { one: "podstrona nie zwróciła", few: "podstrony nie zwróciły", many: "podstron nie zwróciło" })} poprawnej odpowiedzi.` : "Nie zebrano wystarczającej próbki podstron.",
  ...
  checks.push(makeCheck({ id: "security_certificate", category: "security", status: !isHttps ? "not_applicable" : daysToExpiry === null ? "unknown" : daysToExpiry < 7 ? "fail" : daysToExpiry < 30 ? "warning" : "pass", severity: "high", title: "Ważność certyfikatu TLS", observation: daysToExpiry === null ? hasUnavailableDiagnostic("tls_certificate") ? unavailableObservation("terminu ważności certyfikatu TLS", "tls_certificate") : "Nie odczytano terminu ważności certyfikatu." : `Certyfikat jest ważny jeszcze ${formatPolishCount(daysToExpiry, { one: "dzień", few: "dni", many: "dni" })}; wystawca: ${snapshot.tls?.issuer || "nieustalony"}.`, recommendation: "Skonfiguruj automatyczne odnawianie i alarm przed wygaśnięciem certyfikatu.", source: "Publiczne połączenie TLS", weight: 2 }));
  checks.push(makeCheck({ id: "security_tls_protocol", category: "security", status: !snapshot.tls?.protocol ? "unknown" : new Set(["TLSv1.2", "TLSv1.3"]).has(snapshot.tls.protocol) ? "pass" : "fail", severity: "high", title: "Wersja TLS", observation: snapshot.tls?.protocol ? `Uzgodniony protokół: ${snapshot.tls.protocol}.` : hasUnavailableDiagnostic("tls_protocol") ? unavailableObservation("wersji protokołu TLS", "tls_protocol") : "Nie odczytano wersji protokołu TLS.", recommendation: "Wyłącz przestarzałe protokoły i pozostaw TLS 1.2 oraz 1.3.", source: "Publiczne połączenie TLS" }));
  const hstsValue = headerValue(snapshot.headers, "strict-transport-security");
  const hsts = evaluateHsts(hstsValue, isHttps);
  checks.push(makeCheck({ id: "security_hsts", category: "security", ...hsts, severity: "medium", title: "Strict-Transport-Security", source: "Nagłówki odpowiedzi HTTPS" }));
  ...
  checks.push(makeCheck({ id: "security_redirect_chain", category: "security", status: snapshot.redirects.length <= 2 ? "pass" : "warning", severity: "low", title: "Łańcuch przekierowań", observation: `Przed stroną końcową wykonano ${formatPolishCount(snapshot.redirects.length, { one: "przekierowanie", few: "przekierowania", many: "przekierowań" })}.`, recommendation: "Skróć przekierowania do jednego skoku prowadzącego do hosta kanonicznego.", source: "Pasywny pomiar HTTP" }));

  checks.push(makeCheck({ id: "conversion_cta", category: "conversion", status: ctaCount >= 2 ? "pass" : ctaCount === 1 ? "warning" : "fail", severity: "high", title: "Wezwania do działania", observation: `Znaleziono ${formatPolishCount(ctaCount, { one: "czytelny sygnał działania", few: "czytelne sygnały działania", many: "czytelnych sygnałów działania" })}.`, recommendation: "Ustal jedno główne CTA i powtarzaj je w miejscach odpowiadających kolejnym etapom decyzji.", source: "Widoczny tekst strony głównej", weight: 2 }));
  checks.push(makeCheck({ id: "conversion_offer", category: "conversion", status: offerSignals >= 2 ? "pass" : offerSignals === 1 ? "warning" : "fail", severity: "medium", title: "Czytelność oferty", observation: `Znaleziono ${formatPolishCount(offerSignals, { one: "sygnał opisujący ofertę lub korzyści", few: "sygnały opisujące ofertę lub korzyści", many: "sygnałów opisujących ofertę lub korzyści" })}.`, recommendation: "Wyjaśnij dla kogo jest oferta, jaki problem rozwiązuje i co użytkownik otrzyma.", source: "Widoczny tekst strony głównej" }));
  checks.push(makeCheck({ id: "conversion_contact", category: "conversion", status: contactSignals >= 1 ? "pass" : "fail", severity: "high", title: "Dostępność kontaktu", observation: contactSignals ? `Znaleziono ${formatPolishCount(contactSignals, { one: "sygnał kontaktu", few: "sygnały kontaktu", many: "sygnałów kontaktu" })}.` : "Nie znaleziono bezpośredniego sposobu kontaktu.", recommendation: "Dodaj widoczny kontakt lub formularz w kluczowych miejscach ścieżki.", source: "Linki i HTML strony głównej", weight: 2 }));
  ...
  checks.push(makeCheck({ id: "conversion_content_depth", category: "conversion", status: inspection.text.length >= 500 ? "pass" : "warning", severity: "low", title: "Zakres treści decyzyjnej", observation: `Widoczna treść ma ${formatPolishCount(inspection.text.length, { one: "znak", few: "znaki", many: "znaków" })}.`, recommendation: "Dodaj informacje potrzebne do decyzji, ale utrzymaj jasną hierarchię i usuń powtórzenia.", source: "Widoczny tekst strony głównej" }));
  ...
  checks.push(makeCheck({ id: "trust_proof", category: "trust", status: trustSignals >= 2 ? "pass" : trustSignals === 1 ? "warning" : "fail", severity: "medium", title: "Dowody wiarygodności", observation: `Znaleziono ${formatPolishCount(trustSignals, { one: "sygnał realizacji, doświadczenia lub opinii", few: "sygnały realizacji, doświadczenia lub opinii", many: "sygnałów realizacji, doświadczenia lub opinii" })}.`, recommendation: "Pokaż możliwe do zweryfikowania realizacje, proces, opinie lub doświadczenie bez składania nieudokumentowanych obietnic.", source: "Widoczny tekst strony głównej" }));
